...
class HeatDataFormatter(GenericDataFormatter):
    ...
    def split_data(self, df, train_percentage=0.8):
        ...
        # remove cyclic feature names from column definition list
        [self._column_definition.remove(x) for x in self._column_definition if x[0] in self.cyclic_features]
        [self.features.remove(x) for x in self.cyclic_features if x in self.features]

        # Update column definitions to reflect the new cyclic encoded features
        for col in self.encoded_columns:
            if not any([col in x[0] for x in self._column_definition]):
                self._column_definition.append((col, DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT))

        # add column names to column definition
        for i, name in enumerate(self.cat_feature_names):
            if not any([name in x[0] for x in self._column_definition]):
                self._column_definition.append((name, DataTypes.CATEGORICAL, InputTypes.KNOWN_INPUT))
                self.features.append(name)

        # two classes for each one hot encoded categorical feature
        n_categorical_inputs = len(self.cat_feature_names) + len(CATEGORICAL_FEATURES_BINARY)
        self._num_classes_per_cat_input = [2 for _ in range(n_categorical_inputs)]

        # drop categorical columns now that they are encoded
        for entry in [x for x in self._column_definition if x[0] in self.categorical_inputs]:
            self._column_definition.remove(entry)
        [self.features.remove(x) for x in self.categorical_inputs if x in self.features]
    ...
    def set_scalers(self, df, scale_mode='individual'):
        ...
        for col in categorical_inputs:
            ...
            categorical_scalers[col] = OneHotEncoder().fit(
    ...
    def transform_inputs(self, df):
        ...
        for col in cyclic_features:
            # For cyclic features, first get numeric representation
        ...
        if len(categorical_inputs) > 0:
            enc = OneHotEncoder().fit(output[categorical_inputs])
            self._cat_scaler = enc
            cat_feature_names = enc.get_feature_names_out()
            X_enc = pd.DataFrame(enc.transform(output[categorical_inputs]).toarray(), columns=cat_feature_names)
            output = pd.concat([output, X_enc], axis=1)
